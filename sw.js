const C='gengrail-log-v24.11.10-seed-choice-expansion';
  ...
  e.respondWith((async()=>{
   try{
    ...
    selectableBody=selectableBody
      .replace("const PATCH_VERSION='selectable-stream-v1.0.0'","const PATCH_VERSION='selectable-stream-v1.2.0'")
    ...
    const seedLabelPatch=`\n(function(){\n'use strict';\nif(window.__gengrailSeedChoiceLabels120)return;window.__gengrailSeedChoiceLabels120=true;\nfunction relabel(){document.querySelectorAll('.gsel-card').forEach(function(card){const sub=card.querySelector('.gsel-card-sub');if(!sub)return;const txt=sub.textContent||'';const roiTxt=Array.from(card.querySelectorAll('.gsel-grid b')).map(x=>x.textContent||'').find(x=>x.includes('% / '))||'';const m=roiTxt.match(/(-?\\d+)%\\s*\\/\\s*(-?\\d+)%/);if(!m)return;const roi=Number(m[1]),margin=Number(m[2]);if(roi>=15&&margin>=15)sub.textContent=txt.replace(/REVIEW|QUALIFIED|STRONG BUY/g,'VELOCITY BUY');else if(roi>=10&&margin>=10)sub.textContent=txt.replace(/REVIEW|QUALIFIED|STRONG BUY/g,'BUY');else sub.textContent=txt.replace(/QUALIFIED|STRONG BUY/g,'REVIEW');});}\nnew MutationObserver(relabel).observe(document.documentElement,{subtree:true,childList:true});setTimeout(relabel,250);\n})();`;
    const routePatch=`\n(function(){\n'use strict';\nif(window.__gengrailSelectableRouteV120)return;window.__gengrailSelectableRouteV120=true;\ndocument.addEventListener('click',function(e){const btn=e.target&&e.target.closest&&e.target.closest('#grailOpportunityAction');if(!btn)return;if(!(window.GengrailSelectableStream&&typeof window.GengrailSelectableStream.render==='function'))return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();setTimeout(function(){window.GengrailSelectableStream.render();},0);},true);\n})();`;
