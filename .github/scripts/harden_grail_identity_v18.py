...
if 'GENGRAIL GRAIL HUB v1.8' in s:
    print('Grail Hub identity v1.8 already applied')
else:
    s=s.replace('GENGRAIL GRAIL HUB v1.7 — identity-hardened discovery',
                'GENGRAIL GRAIL HUB v1.8 — language + era identity hardening')
    s=s.replace('version:4,','version:5,',1)

    anchor="""function identityStatus(t,cardNumber,family){
...
function identityStatus(t,cardNumber,family,language){
  ...
  return 'CONFIRMED';
    ...
    if anchor not in s:
        raise SystemExit('Safety stop: identityStatus anchor not found')
    s=s.replace(anchor,replacement,1)

    old_parse="""  const type=grader?'GRADED':'RAW',family=printingFamily(t,cardNumber),identity=identityStatus(t,cardNumber,family),condition=type==='RAW'?rawCondition(title,item?.condition||''):'GRADED';
  return {item,title,t,cardNumber,grader,grade,type,printingFamily:family,identityStatus:identity,condition,ask,inbound:shippingPrice(item),tokens:tokensFor(title)};"""
    new_parse="""  const type=grader?'GRADED':'RAW',family=printingFamily(t,cardNumber),language=languageFrom(t),identity=identityStatus(t,cardNumber,family,language),condition=type==='RAW'?rawCondition(title,item?.condition||''):'GRADED';
  return {item,title,t,cardNumber,grader,grade,type,printingFamily:family,language,identityStatus:identity,condition,ask,inbound:shippingPrice(item),tokens:tokensFor(title)};"""
    if old_parse not in s:
        raise SystemExit('Safety stop: parseListing anchor not found')
    s=s.replace(old_parse,new_parse,1)

    old_peer="""    if(x===row||x.cardNumber!==row.cardNumber||x.grader!==row.grader||x.grade!==row.grade)return false;
    if(x.printingFamily!==row.printingFamily)return false;
    if(row.type==='RAW'&&x.condition!==row.condition)return false;"""
    new_peer="""    if(x===row||x.cardNumber!==row.cardNumber||x.grader!==row.grader||x.grade!==row.grade)return false;
    if(x.printingFamily!==row.printingFamily||x.language!==row.language)return false;
    if(row.type==='RAW'&&x.condition!==row.condition)return false;"""
    if old_peer not in s:
        raise SystemExit('Safety stop: peer key anchor not found')
    s=s.replace(old_peer,new_peer,1)

    # Surface the resolved language beside condition/set on each listing.
